...
WAVELENGTH = 0.095  # м (9.5 см для S-диапазона КОНДОР ФКА)
...
def calculate_angles(R_s: Tuple[float, float, float], 
    ...
    # Новые расчеты углов
    grazing_angle = 90 - ay_grad  # Угол скольжения
    incidence_angle = ay_grad     # Угол падения
...
def create_orbital_track(tracker: SatelliteTracker, 
                        ...
                        Fd: float = 0) -> Tuple[List[Tuple], List[Dict]]:
...
def _test():
    ...
    with shapefile.Writer(filename, shapefile.POINT) as track_shape:
        ...
        for Fd in Fd_values:
            t_semki, points_data = create_orbital_track(tracker, params, track_shape, Fd)
    ...
    incidence = [p['incidence'] for p in all_points]
    
    # Создание scatter plot
    plt.scatter(y_grad, grazing, s=10, c='blue', label='Угол скольжения', alpha=0.5)
    plt.scatter(y_grad, incidence, s=10, c='red', label='Угол падения', alpha=0.5)
    
    # Настройки графиков
    plt.xlabel('Угол визирования ϒ, градусы')
    plt.ylabel('Значение угла, градусы')
    plt.title('Зависимость углов скольжения и падения от угла визирования')
    plt.legend()
    plt.grid(True)
    plt.xlim(24, 55)
    
    # Сохранение и отображение
    plt.savefig('angles_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
