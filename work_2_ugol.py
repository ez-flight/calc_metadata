...
def create_orbital_track(tracker: SatelliteTracker, 
    ...
    while dt < params.dt_end:
        ...
        if 24 < angles['y_grad'] < 55 and angles['R_0'] < angles['R_e']:
            ...
            # Расчет частоты доплера Fd
            Fd = calc_f_doplera(a, WAVELENGTH, math.radians(angles['ay_grad']), 
...
def _test():
    ...
    with shapefile.Writer(filename, shapefile.POINT) as track_shape:
        ...
        a_values = range(-10000, 10001, 5000)
        results = {}
        
        for a in a_values:
            t_semki = create_orbital_track(tracker, params, track_shape, a)
            results[a] = t_semki
            print(f"\nРезультаты для a = {a} Град:")
